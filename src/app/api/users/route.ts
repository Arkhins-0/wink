...
import { canCreateRole, descendants } from "@/lib/hierarchy";
...
/** Everyone below the signed-in person. `?role=` narrows it. */
export const GET = handle(async (request) => {
  const user = await requireUser();
  const role = new URL(request.url).searchParams.get("role");
  const people = (await descendants(user)).filter((p) => !role || p.role === role);
